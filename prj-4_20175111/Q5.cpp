...
class Trasure :public Sprite {
public:
	Trasure(int x, int y) :Sprite(x, y, 'T') {}
	void move(char d) {}
};
...
int main() {
	...
	list.push_back(new Trasure(height - 1, width - 1));
	list.push_back(new Enemy(3, 3));

	drawLine('*');
	cout << "이 게임의 목표는 함정(T)나 적(E)를 만나지 않고 보물에 도달하는 것이다." << endl;
	cout << "주인공은 w,a,s,d 키를 이용하여 움직일 수 있다." << endl;
	drawLine('*');
	cout << endl;
